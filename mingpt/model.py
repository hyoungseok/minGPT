...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.tok_emb = nn.Embedding(config.vocab_size, config.n_emb)
        self.pos_emb = nn.Parameter(torch.zeros(1, config.block_size, config.n_emb), requires_grad=True)
        self.drop = nn.Dropout(config.emb_pdrop)
        # transformer
        self.blocks = nn.Sequential(*[Block(config) for _ in range(config.n_layer)])
        # decoder head
        self.ln_f = nn.LayerNorm(config.n_emb)
        self.head = nn.Linear(config.n_emb, config.vocab_size, bias=False)
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            loss = ftn.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1))
